...
@RestController
@RequestMapping("/api")
public class PrecioEquipajeController {

    @Autowired
    private PrecioEquipajeService service;

    @PostMapping(value = "/precioEquipaje/save", consumes = { MediaType.MULTIPART_FORM_DATA_VALUE })
    public ResponseEntity<Response> save(@Validated(onCreate.class) @ModelAttribute PrecioEquipaje precio,
            HttpServletRequest request) {
        return service.save(precio, request);
    }
   
    @PostMapping(value = "/precioEquipaje/index", consumes = { "application/xml", "application/json" })
    ...
    @PostMapping(value = "/precioEquipaje/update", consumes = { MediaType.MULTIPART_FORM_DATA_VALUE })
    public ResponseEntity<Response> update(@Valid @ModelAttribute @Validated(onCreate.class) PrecioEquipaje precio,
            HttpServletRequest request) {
        return service.update(precio, request);
    }

    @PostMapping(value = "/precioEquipaje/{id}/delete")
    public ResponseEntity<Response> delete(@PathVariable(name = "id", required = true) int id,
            HttpServletRequest request) {
        return service.delete(id, request);
    }

    @GetMapping(value = "/precioEquipaje/{id}/detail")
