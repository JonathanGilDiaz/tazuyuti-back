...
@RestController
@RequestMapping("/api")
public class PrecioPaqueteriaController {

    @Autowired
    private PrecioPaqueteriaService service;

    @PostMapping(value = "/precioPaqueteria/save", consumes = { MediaType.MULTIPART_FORM_DATA_VALUE })
    public ResponseEntity<Response> save(@Validated(onCreate.class) @ModelAttribute PrecioPaqueteria precio,
            HttpServletRequest request) {
        return service.save(precio, request);
    }
   
    @PostMapping(value = "/precioPaqueteria/index", consumes = { "application/xml", "application/json" })
    public ResponseEntity<Response> index(
        ...
        try {
            ...
            String message = Utils.validateFilteringInformation(
                    SystemText.Precios.OPCIONES_VALIDAS_PAGINACION, request.getSort(),
                    request.getFilters());
            if (message.equals("")) {
    ...
    @PostMapping(value = "/precioPaqueteria/update", consumes = { MediaType.MULTIPART_FORM_DATA_VALUE })
    public ResponseEntity<Response> update(@Valid @ModelAttribute @Validated(onCreate.class) PrecioPaqueteria precio,
            HttpServletRequest request) {
        return service.update(precio, request);
    }

    @PostMapping(value = "/precioPaqueteria/{id}/delete")
    public ResponseEntity<Response> delete(@PathVariable(name = "id", required = true) int id,
            HttpServletRequest request) {
        return service.delete(id, request);
    }

    @GetMapping(value = "/precioPaqueteria/{id}/detail")
    public ResponseEntity<Response> detail(@PathVariable(name = "id", required = true) int id) {
        return service.detail(id);
    }

    
    @GetMapping(value = "/precioPaqueteria/getAll")
