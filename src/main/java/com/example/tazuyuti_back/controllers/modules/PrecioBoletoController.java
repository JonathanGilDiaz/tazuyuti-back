...
@RestController
@RequestMapping("/api")
public class PrecioBoletoController {

    @Autowired
    private PrecioBoletoService service;

    @PostMapping(value = "/precioBoleto/save", consumes = { MediaType.MULTIPART_FORM_DATA_VALUE })
    public ResponseEntity<Response> save(@Validated(onCreate.class) @ModelAttribute PrecioBoleto precio,
            HttpServletRequest request) {
        return service.save(precio, request);
    }

    @PostMapping(value = "/precioBoleto/index", consumes = { "application/xml", "application/json" })
    public ResponseEntity<Response> index(
        ...
        try {
            ...
            String message = Utils.validateFilteringInformation(
                    SystemText.PrecioBoleto.OPCIONES_VALIDAS_PAGINACION, request.getSort(),
                    request.getFilters());
            if (message.equals("")) {
    ...
    @PostMapping(value = "/precioBoleto/update", consumes = { MediaType.MULTIPART_FORM_DATA_VALUE })
    public ResponseEntity<Response> update(@Valid @ModelAttribute @Validated(onCreate.class) PrecioBoleto precio,
            HttpServletRequest request) {
        return service.update(precio, request);
    }

    @PostMapping(value = "/precioBoleto/{id}/delete")
    public ResponseEntity<Response> delete(@PathVariable(name = "id", required = true) int id,
            HttpServletRequest request) {
        return service.delete(id, request);
    }

    @GetMapping(value = "/precioBoleto/{id}/detail")
    public ResponseEntity<Response> detail(@PathVariable(name = "id", required = true) int id) {
        return service.detail(id);
    }

    @GetMapping(value = "/precioBoleto/getAll")
